    ...
    def test_chat_completions_stream(self, openai_client):
        ...
        try:
            ...
            for chunk in stream:
                ...
                assert chunk.object == "chat.completion.chunk", "Incorrect response object type"
...
async def test_retry_canceled_stream_chat_completion(async_client: AsyncClient):
    ...
    try:
        async with async_client.stream(
            "POST", "/v1/chat/completions", json=payload, timeout=5
        ) as response:
    ...
    async with async_client.stream(
        "POST", "/v1/chat/completions", json=payload, timeout=15
    ) as response:
        response.raise_for_status()
        async for line in response.aiter_lines():
            if line:
        ...
        if line.startswith("data:"):
            ...
            if data_part and data_part != "[DONE]":
                try:
                    ...
                    if "choices" in chunk_json and chunk_json["choices"][0].get("delta", {}).get(
                        "content"
                    ):
