...
from mlx_omni_server.chat.schema import (ChatCompletionChunk, ChatCompletionRequest,
...
async def test_responses_streaming_tool_call(mock_create_model, async_client):
    ...
    deltas = [data["delta"] for event, data in events if event == "response.function_call_arguments.delta"]
