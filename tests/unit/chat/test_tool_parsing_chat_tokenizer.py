...
from mlx_omni_server.chat.schema import FunctionCall, Role, Tool, ToolCall
        ...
        if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
            ...
            tool_call = ToolCall(
                ...
                function=FunctionCall(
                    name="test_function", arguments=f'{{"param": "{tool_content_str}"}}'
                ),
            )
...
class TestToolParsingChatTokenizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tokenizer.tool_parser = MockToolParser(
            start_token="<tool_code>", end_token="</tool_code>"
        )
    ...
    def test_tool_call_in_single_chunk(self):
        ...
        self.assertEqual(
            final_msg.tool_calls[0].function.arguments, '{"param": "print(\'hello\')"}'
        )
    ...
    def test_decode_stream_buffer_release_on_overflow(self):
        ...
        self.assertTrue(
            len(self.tokenizer.buffer) < len(self.tokenizer.tool_parser.tool_call_start_token)
        )
    ...
    def test_no_extra_characters_with_tool_token(self):
        ...
        all_content = "".join(
            c.content for c in [result1, result2, final_result] if c and c.content
        )
    ...
    def test_no_extra_less_than_at_end(self):
        ...
        all_content = "".join(
            c.content for c in [result1, result2, result3, final_result] if c and c.content
        )
