...
from mlx_omni_server.chat.tools.llama3 import Llama3ChatTokenizer
from mlx_omni_server.chat.tools.mistral import MistralChatTokenizer
...
class TestMistralChatTokenizer(unittest.TestCase):
    ...
    def test_mistral_decode_single_tool_call(self):
        ...
        self.assertEqual(json.loads(tool_call.function.arguments), {"location": "Boston, MA"})
    ...
    def test_mistral_decode_multiple_tool_calls(self):
        ...
        self.assertEqual(json.loads(tool_call1.function.arguments), {"location": "Boston, MA"})

        # Check second tool call
        tool_call2 = result.tool_calls[1]
        self.assertEqual(tool_call2.function.name, "get_forecast")
        self.assertEqual(json.loads(tool_call2.function.arguments), {"location": "New York, NY"})
    ...
    def test_mistral_decode_invalid_tool_call(self):
        ...
        self.assertIsNotNone(result)
        self.assertEqual(result.role, Role.ASSISTANT)
        self.assertEqual(result.content, text)  # Should return original text for invalid format
        self.assertIsNone(result.tool_calls)
