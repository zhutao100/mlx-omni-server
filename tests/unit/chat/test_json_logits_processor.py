...
from mlx_omni_server.chat.mlx_lm.json_logits_processor import JsonLogitsProcessor
...
class TestJsonLogitsProcessor:
    ...
    def test_init_valid_inputs(self, mock_tokenizer, valid_response_format):
        ...
        with (
            patch(
                "mlx_omni_server.chat.mlx_lm.json_logits_processor.JsonSchemaParser"
            ) as mock_parser,
            patch(
                "mlx_omni_server.chat.mlx_lm.json_logits_processor.build_transformers_prefix_allowed_tokens_fn"
            ) as mock_build_fn,
        ):
    ...
    def test_init_invalid_response_format_type(self, mock_tokenizer):
        ...
        with pytest.raises(
            ValueError, match="JsonLogitsProcessor only supports type='json_schema'"
        ):
    ...
    def test_init_missing_json_schema(self, mock_tokenizer):
        ...
        with pytest.raises(
            ValueError, match="JsonLogitsProcessor requires a json_schema in response_format"
        ):
            JsonLogitsProcessor(mock_tokenizer, response_format)
    ...
    def test_init_calls_prefix_function_builder(self, mock_tokenizer, valid_response_format):
        ...
        with (
            ...
            mock_build_fn.assert_called_once_with(
                mock_tokenizer._tokenizer, mock_parser.return_value
            )
    ...
    def test_call_with_1d_tokens(self, mock_tokenizer, valid_response_format):
        ...
        with (
            patch("mlx_omni_server.chat.mlx_lm.json_logits_processor.JsonSchemaParser"),
            patch(
                "mlx_omni_server.chat.mlx_lm.json_logits_processor.build_transformers_prefix_allowed_tokens_fn"
            ) as mock_build_fn,
        ):
...
class TestJsonLogitsProcessorEdgeCases:
    ...
    def simple_response_format(self):
        ...
        json_schema_format = JsonSchemaFormat(name="simple", schema={"type": "string"})
        return ResponseFormat(type="json_schema", json_schema=json_schema_format)
    ...
    def test_error_handling_with_malformed_schema(self, mock_tokenizer):
        ...
        malformed_schema = {"name": "malformed", "schema": {"type": "object"}}
    ...
    def test_json_logits_processor_with_complex_schema(self, mock_tokenizer):
        ...
        response_format = ResponseFormat(
            type="json_schema",
            json_schema=JsonSchemaFormat(name="complex_schema", schema=json_schema),
        )
...
class TestJsonLogitsProcessorIntegration:
    ...
    def test_json_logits_processor_creation_in_model(self, mock_tokenizer, chat_completion_request):
        """Test that JsonLogitsProcessor is correctly created in the model system."""
        generate_kwargs = {}

        if (
            chat_completion_request.response_format
            and chat_completion_request.response_format.json_schema
        ):
            generate_kwargs["logits_processors"] = [
                JsonLogitsProcessor(mock_tokenizer, chat_completion_request.response_format)
            ]
    ...
    def test_json_logits_processor_with_real_tokenizer_simulation(
        self, mock_tokenizer, simple_json_schema
    ):
        """Test JsonLogitsProcessor with a more realistic tokenizer simulation."""
        mock_tokenizer._tokenizer.convert_ids_to_tokens = Mock(
            return_value=["<s>", "hello", "world", "</s>"]
        )
        mock_tokenizer._tokenizer.convert_tokens_to_ids = Mock(
            return_value=lambda tokens: [0, 1, 2, 3]
        )
    ...
    def test_json_logits_processor_schema_validation(self, mock_tokenizer):
    ...
    def test_json_logits_processor_error_scenarios(self, mock_tokenizer):
    ...
    def test_json_logits_processor_performance_characteristics(self, mock_tokenizer):
        ...
        with (
            ...
            for token_count, vocab_size in test_sizes:
                ...
                logger.info(
                    f"Processed {token_count} tokens, {vocab_size} vocab size in {processing_time:.4f}s"
                )
    ...
    def test_json_logits_processor_thread_safety(self, mock_tokenizer):
        """Test thread safety of JsonLogitsProcessor."""
        json_schema_format = JsonSchemaFormat(name="thread_test", schema={"type": "string"})
        response_format = ResponseFormat(type="json_schema", json_schema=json_schema_format)
