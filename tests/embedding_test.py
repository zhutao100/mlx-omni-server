...
def test_embeddings_multiple_texts(openai_client):
    ...
    try:
        ...
        # Basic validation
        assert response is not None, "No response received"
        assert len(response.data) == len(inputs), "Number of embeddings should match number of input texts"
        assert response.model == model, "Model name in response should match request"
        
        # Validate each embedding vector
        for i, embedding_data in enumerate(response.data):
            assert embedding_data.embedding is not None, f"Embedding {i+1} should not be empty"
            assert len(embedding_data.embedding) > 0, f"Embedding {i+1} should contain vector data"
...
def test_embeddings_with_dimensions(openai_client):
    ...
    try:
        ...
        response = openai_client.embeddings.create(
            model=model,
            input="Testing dimensions parameter",
...
def test_embeddings_missing_model(client):
    ...
    try:
        ...
        logger.info(f"Missing Model Response: {response.status_code}, {response.json()}")
        
        # Validate response
        assert response.status_code == 422, "Missing required 'model' parameter should return validation error"
...
def test_embeddings_missing_input(client):
    ...
    try:
        ...
        # Send request
        response = client.post("/v1/embeddings", json=request_data)
        logger.info(f"Missing Input Response: {response.status_code}, {response.json()}")
        
        # Validate response
        assert response.status_code == 422, "Missing required 'input' parameter should return validation error"
