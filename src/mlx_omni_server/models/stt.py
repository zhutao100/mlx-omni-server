...
class STTRequestForm:
    def __init__(
        ...
        response_format: Optional[ResponseFormat] = Form(ResponseFormat.JSON, description="The format of the transcription output"),
        ...
        if timestamp_granularities:
            for gran in timestamp_granularities:
                try:
                    self.timestamp_granularities.append(TimestampGranularity(gran.lower()))
                except ValueError:
                    raise ValueError(f"Invalid timestamp granularity: {gran}. Must be one of: word, segment")
        else:
    ...
    def validate(self):
        ...
        if (self.timestamp_granularities and
            TimestampGranularity.WORD in self.timestamp_granularities and
            self.response_format != ResponseFormat.VERBOSE_JSON):
            raise ValueError('Word-level timestamps require response_format=verbose_json')
