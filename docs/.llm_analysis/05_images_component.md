...
-   **Schema:** The `ImageGenerationRequest` schema is compatible with the OpenAI API, supporting parameters like `prompt`, `model`, `n`, `size`, and `response_format`. It defaults to `filipstrand/Z-Image-Turbo-mflux-4bit`.
