...
-   **Lifecycle Management:** The application uses a `lifespan` manager to run background tasks. Currently it starts `background_cache_cleanup()` from the chat subsystem, which periodically evicts chat response cache entries.
