...
-   **`mlx_lm/` & `mlx_vlm/`:** These contain the abstraction layers that wrap the `mlx-lm` and `mlx-vlm` libraries, providing a consistent interface for text and multimodal models. `mlx_vlm/` wires in `mlx-vlm`’s `PromptCacheBundle` (KV + optional multimodal decode context + model-state like `_rope_deltas`) and enforces append-only reuse when continuing without re-sending media. For GLM4V, the server expands the `<|image|>` placeholder into the processor’s patch-grid token count so cache tokens match the actual `input_ids`. If the model emits thinking markup but the prompt template omits prior reasoning (GLM4/GLM4V render history as `<think></think>`), reuse will stop at the first `<think>` divergence; the server trims KV to the shared prefix and re-processes the suffix. Full reuse across turns requires either disabling thinking (`enable_thinking=false`) or changing the template/client behavior to round-trip the exact assistant tokens.
